...
//注意，这里我修改了原来的函数原型，添加了在builtin_cmd函数和do_bgfg函数的参数列表中添加了argv，表示输入指令的数目，
//同时修改了parseline函数，使得它能够它argv数目通过argcp指针正确传递回来。
...
int parseline(const char *cmdline, char **argv, int* argcp);
...
int builtin_cmd(char **argv, int argc)
{
    ...
    if(!strcmp(cmd, "quit")){
        //直接退出，后面需要调用kill函数给所有的后台runing或者stop的进程发送-9信号
        exit(0);
    }
...
void do_bgfg(char **argv, int argc)
{
    //其实这里应该加上错误判断，比如使用输入了三个参数，这里暂时不考虑,这里默认用户会遵循规则
    if(argc != 2){
...
void waitfg(pid_t pid)
{
    ...
    //FGPID = 0;
    fg_stop_or_exit = 0;
    //让SIGCHLD信号处理程序，完成回收后，需要将全局pid变量置位为当前前台pid，
    //也就是说如果这个时候是别的后台进程终止了，也要进行处理，但是这个时候handle程序不会置位pid。
 ...
 * 按照书上的改进而来，在deletejob的基础上，判断当前回收的子进程是否是前台程序，如果是直接置位全局变量pid
 */
void sigchld_handler(int sig)
{
    int olderrno = errno;
